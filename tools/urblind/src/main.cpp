...
/**
 * ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
 * │ macOS screen capture wrapper using ScreenCaptureKit (macOS 12.3+)                                                │
 * └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
 */
...
int main(int argc, char* argv[]) {
  ...
  const float smoothingFactor = 15.0f;
  ...
  if (selectedMonitor == -1) {
    ...
    if (selectedMonitor == -1) {
      std::cerr << "Invalid monitor index! Falling back to monitor 0.\n";
      selectedMonitor = 0;
    }
  }
  ...
  while (!shouldClose) {
    ...
    bool isZooming = std::abs(targetZoom - zoom) > 0.001f;

    if (isZooming) {
      // Get the texture coordinate under the mouse BEFORE updating zoom
      Vector2 mouseOnTex = GetMousePositionOnTexture(mousePosition, pan, zoom);
      // Smooth zoom
      zoom += (targetZoom - zoom) * smoothing;
      // Directly compute pan to keep the same texture point under the cursor
      pan.x = mouseOnTex.x - (mousePosition.x / zoom);
      pan.y = mouseOnTex.y - (mousePosition.y / zoom);
      targetPan = pan;
    } else {
      zoom = targetZoom;
